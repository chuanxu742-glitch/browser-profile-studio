...
import { resolveCdpProfile } from './profile.js';
...
export async function startCdpService(env: NodeJS.ProcessEnv = process.env) {
  ...
  const maxConnections = z.coerce.number().int().min(1).max(64).parse(env.CDP_MAX_CONNECTIONS ?? '8');
  ...
  const metadata = await resolveCdpProfile(directory, env);
  ...
  const server = createServer((req, res) => {
    ...
    if (req.method === 'GET' && req.url === '/status') {
      res.end(JSON.stringify({ ready: !stopping, configured: metadata, connections: upstreams.size, maxConnections })); return;
    }
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    if (upstreams.size >= maxConnections) {
      socket.end('HTTP/1.1 429 Too Many Requests\r\nConnection: close\r\nContent-Length: 0\r\n\r\n'); return;
    }
    const remote = new WebSocket(upstream, { maxPayload: 16 * 1024 * 1024, handshakeTimeout: 5000 });
    upstreams.add(remote);
    peers.add(remote);
    remote.on('error', () => socket.destroy());
    remote.on('close', () => { upstreams.delete(remote); peers.delete(remote); socket.destroy(); });
